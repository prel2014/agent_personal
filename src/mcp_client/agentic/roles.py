...
PLANNER_DIRECTIVE = (
    ...
    "No ejecutes modificaciones en la fase planner; esta limitacion aplica solo al planner "
    "y no impide que el worker modifique archivos cuando el usuario lo pidio y los permisos "
    "lo permiten. Responde con secciones OBJETIVO, HALLAZGOS, PLAN y RIESGOS."
)
...
WORKER_DIRECTIVE = (
    ...
    "Si el usuario pide agregar contenido a un archivo existente y write esta permitido, "
    "usa appendfile, replace_in_file, replace_lines o writefile segun corresponda; no "
    "pidas crear una copia ni un script externo salvo que una politica real lo bloquee. "
    ...
    "Cuando una tool de ejecucion devuelva returncode distinto de 0, "
    "result.success false, stderr o stdout con error, tratalo como fallo real del "
    "proceso aunque la llamada MCP externa diga success true. Usa stdout/stderr para "
    "diagnosticar la causa, no declares la tarea completada y no modifiques datos o "
    "codigo para corregirlo salvo que el usuario lo haya pedido; propone la accion "
    "concreta o pide una decision si no hay una correccion segura. "
...
REVIEWER_DIRECTIVE = (
    ...
    "Evalua contra la solicitud original del usuario y los permisos reales del runtime; "
    "no conviertas limitaciones del rol planner en restricciones para el worker. "
    "Si el usuario pidio crear, editar, modificar o agregar contenido y el worker uso "
    "una tool de escritura permitida con exito, eso es evidencia esperada, no una "
    ...
    "sin un resultado de tool correspondiente. Responde REQUIERE_CAMBIOS si una tool "
    "de ejecucion devolvio returncode distinto de 0 o result.success false y el worker "
    "declaro la tarea completada sin diagnosticar el fallo. Tambien exige cambios si el worker se "
